...
3. **Food lookup table (as coded, `PROTEIN_FOODS` — 20 items, 11 everyday + 9 hawker; grams are whole numbers and live in code so the two languages cannot drift, with only labels translated):**
   ...
   | `chickenRiceSteamed` | Chicken rice — steamed chicken | 21 | hawker |
   | `chickenRiceRoasted` | Chicken rice — roasted chicken | 29 | hawker |
   ...
   All 20 keys resolve to a label in `en.ts`; none is missing. `tallyTotal` is a plain sum of `count × grams` over these plus any user-added items.
...
| `chickenRice` — "Chicken rice — breast, less rice" | 30 g | 5.95 (steamed) / 8.22 (roasted) | **504 g** steamed, **365 g** roasted | **Row split — see below.** One row could not carry both densities: 30 g implies a 504 g plate if steamed or a 365 g plate if roasted, and those cannot both be the dish someone ate. Replaced by the two rows below. |
| `chickenRiceSteamed` — "Chicken rice — steamed chicken" | **21 g** *(new)* | 5.95 g/100 g | 350 g plate *(assumed)* | **Density sourced, serving weight assumed.** 5.95 × 3.5 = 20.8 → 21 g. |
| `chickenRiceRoasted` — "Chicken rice — roasted chicken" | **29 g** *(new)* | 8.22 g/100 g | 350 g plate *(assumed)* | **Density sourced, serving weight assumed.** 8.22 × 3.5 = 28.8 → 29 g. Lands within 1 g of the old single row, so anyone who ate roasted was already being counted about right; it was steamed that was over-counted by ~8 g. |
...
**The one real defect these four records expose is in `chickenRice`.** HPB measures roasted and steamed chicken rice as separate dishes and they differ by **38%** (8.22 vs 5.95 g/100 g) — far wider than the whole-gram rounding the rest of the table is built on. Our table has **one** chicken rice row, so whichever variant its figure describes, it is wrong by roughly 8 g for anyone who ate the other one. **Fixed 2026-08-17 — the row was split.** `chickenRice` (30 g) became `chickenRiceSteamed` (21 g) and `chickenRiceRoasted` (29 g), both derived from HPB's densities on **one shared assumption: a 350 g plate, edible portion.** That assumption is the only unsourced part of the pair and is stated in the code comment as well as here; HPB's own serving-size field, if it carries one, would replace it. Changed in `lib/assessments/proteinCalculator.ts`, `lib/i18n/en.ts`, `lib/i18n/zh.ts` (白鸡饭 / 烧鸡饭) and both `proageing-site` pages.

**A second table had to be aligned with it.** The `hawker-protein` how-to (`en.ts`/`zh.ts` → `howTo['hawker-protein'].portions`, rendered by `components/howto/HowToPanel.tsx`) carries its own protein figures, and its chicken rice row also read 30 g. Left alone, the app would have shown two different numbers for the same dish on two different screens. That row is prescriptive — the how-to's whole point is *what to order* — so it now reads **"Chicken rice — steamed breast, less rice, add egg" at 28 g**: the sourced steamed figure (21) plus the sourced egg figure (7). Bundling the add-on into the row is that table's existing convention ("Fish soup + egg + tofu", "+ extra wanton"). Worth knowing that this second table exists: **it is a separate list of protein values from `PROTEIN_FOODS` and nothing in the type system ties the two together**, so a change to one silently leaves the other behind. It was found by grep, not by a failing check. `tallyTotal` iterates `PROTEIN_FOODS`, so a stale `chickenRice` count in someone's `localStorage` is silently ignored rather than mis-added — and the tally resets daily in any case, so the old key cannot outlive the day of the deploy.

> **Caveat on the 21/29 spread — the shared plate weight is the weak link, and it may understate how close the two really are.** Both figures apply the *same* 350 g plate to different densities, which assumes a steamed and a roasted plate weigh the same. Part of HPB's 38% density gap is probably **water**: roasting drives off moisture, so the same amount of chicken weighs less and every nutrient per 100 g rises. If a roasted plate is genuinely lighter than a steamed one, the per-plate protein difference is smaller than 8 g — possibly much smaller. The split itself is still correct (one row demonstrably could not serve both dishes), but the *size* of the gap between the two rows is the least certain number in this section. HPB's per-serving figures, if the database carries them, would settle it directly and should be preferred over our density × plate-weight arithmetic the moment they are available.

**This is now checked by a script, not by this paragraph.** `npm run verify:protein` (`scripts/verify-protein-table.mjs`) re-derives every mapped row as `round(density × serving ÷ 100)` from `data/hpb-food-insights.csv` and `lib/assessments/proteinSources.ts`, and exits non-zero if a committed figure no longer follows. The four HPB records above are vendored into that CSV verbatim, provenance flags included, so verification needs no network — which matters, because `hpb.gov.sg`, USDA FoodData Central and OpenFoodFacts all fail to connect from the sandbox. Proven against seven injected faults: a changed gram value, a changed serving weight, an altered transcription, a mapping pointing at a missing record, duplicate record names, a blank density, and a deleted CSV were each caught. A mistyped food key is caught earlier still, by `tsc` — `PROTEIN_FOODS` is `as const satisfies`, so its keys form a real union (`ProteinFoodKey`) and `PROTEIN_SOURCES` is a `Partial<Record<…>>` over it.
